  ...
  for (const c of sc.cities) {
    ...
    for (let i = 1; i <= k; i++) {
      ...
      sc.services.push({
        ...
        daysPerWeek: 5, shiftHours: 8, shifts: 1,
        readyDate: '2026-09-01', yard: 10
      });
    }
  }
})(DEFAULT_SCENARIO);
...
function runSim(sc) {
  ...
  const endD = parseD(sc.calendar.simEnd);
  ...
  for (let i = 0; i < nDays; i++) {
    ...
    for (const s of svc) {
      ...
      if (c.hub) {
        ...
        s.ready -= inst;
        const yard = (p.yard == null ? 10 : +p.yard);
        ...
        hubLoad[i] += rem;               // местный хаб режет в тот же день
      } else {
    ...
    // хаб: подготовка региональных бамперов и отправка туда, где нужнее
    if (isHubWorkday(sc, date, ds, holidays)) {
      let todays = 0;
      toHub = toHub.filter(ev => { if (ev.arrive <= i) { todays += ev.qty; return false; } return true; });
      if (todays > 0) {
        hubLoad[i] += todays;
        hubStock += todays * (1 - (+sc.bumpers.scrapPct || 0) / 100);
      }
      if (hubStock > 1e-9) {
  ...
  const refLine = capacityOf(sc, { posts: 2, lifts: 1, mechanics: 5, shiftHours: 8, shifts: 1 }, 1).cap * 5;
